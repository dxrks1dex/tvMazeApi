...
export function CardContainer(item: itemType): JSX.Element {
  ...
  useEffect(() => {
    setIsElementOut(
      isElementOutOfPage({ elementRef: tooltipRef, isTooltipVisible }),
    );
  }, [isTooltipVisible]);
  ...
  return (
    <FilmComponentStyle>
      <div onMouseLeave={handleMouseLeave} onMouseEnter={handleMouseEnter}>
        <StyledLink href={`/${locale}/show?id=${item?.id}`} locale={locale}>
          <FilmImage
            src={
              item?.image?.original ??
        ...
        {isTooltipVisible && (
          <Tooltip
            ref={tooltipRef}
            style={
              isElementOut ? { marginLeft: "-215px" } : { marginLeft: "200px" }
            }
          >
            <div
